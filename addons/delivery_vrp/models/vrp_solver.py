...
class VrpSolver(models.AbstractModel):
    _name = 'vrp.solver'
    _description = 'Solver VRP avec OptaPlanner'
    ...
    def solve_vrp(self, optimization_record):
        ...
        """
        try:
            # Import OptaPlanner (géré de manière sécurisée)
            from .vrp_optaplanner_solver import VrpOptaPlannerSolver
            ...
            _logger.error('OptaPlanner n\'est pas installé')
            return {
                'success': False,
                'error': 'OptaPlanner n\'est pas installé. Veuillez l\'installer avec: pip install optaplanner'
            }
